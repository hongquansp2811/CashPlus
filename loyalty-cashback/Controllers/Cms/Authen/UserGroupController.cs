...
namespace LOYALTY.Controllers
{
    ...
    public class UserGroupController : ControllerBase
    {
        ...
        private static string functions = "cms_authen_groupuser";
        public UserGroupController(IConfiguration configuration, IUserGroup UserGroup, ILoggingHelpers logging)
        {
        ...
        [HttpPost]
        public JsonResult GetList(UserGroupRequest request)
        {
            ...
            APIResponse data = _UserGroup.getList(request);
        ...
        public JsonResult GetDetail(Guid id)
        {
            ...
            APIResponse data = _UserGroup.getDetail(id);
        ...
        [Authorize(Policy = "WebAdminUser")]
        [HttpPost]
        public JsonResult Create(UserGroupRequest request)
        {
            var username = User.Claims.Where(p => p.Type.Equals(ClaimTypes.Name)).FirstOrDefault();
            var list_permission = User.Claims.Where(p => p.Type.Equals(ClaimTypes.Role)).Select(p => p.Value).FirstOrDefault();
            ...
            APIResponse data = _UserGroup.create(request, username.Value);
        ...
        [Authorize(Policy = "WebAdminUser")]
        [HttpPost]
        public JsonResult Update(UserGroupRequest request)
        {
            var username = User.Claims.Where(p => p.Type.Equals(ClaimTypes.Name)).FirstOrDefault();
            var list_permission = User.Claims.Where(p => p.Type.Equals(ClaimTypes.Role)).Select(p => p.Value).FirstOrDefault();
            ...
            APIResponse data = _UserGroup.update(request, username.Value);
        ...
        public JsonResult Delete(DeleteGuidRequest req)
        {
            ...
            APIResponse data = _UserGroup.delete(req);
            // Ghi log
            if (data.code == "200")
            {
                var remoteIP = Request.HttpContext.Connection.RemoteIpAddress;
        ...
        {
            var username = User.Claims.Where(p => p.Type.Equals(ClaimTypes.Name)).FirstOrDefault();
            APIResponse data = _UserGroup.getPermission(id);
