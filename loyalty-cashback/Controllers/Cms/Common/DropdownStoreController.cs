...
{
    [Route("api/store/dropdownStore")]
    [Authorize(Policy = "WebPartnerUser")]
    [ApiController]
    public class DropdownStoreController : ControllerBase
    {
        ...
        public DropdownStoreController(IConfiguration configuration, IOtherListType otherListType, ILoggingHelpers logging, LOYALTYContext context)
        {
        ...
        public JsonResult GetListUserAdmin()
        {
            ...
            var is_admin = _context.Users.Where(p => p.id == userId).Select(p => p.is_admin).FirstOrDefault();
            if (is_admin == false)
            {
                return new JsonResult(Consts.Error_Permissions) { StatusCode = 222 };
            }
